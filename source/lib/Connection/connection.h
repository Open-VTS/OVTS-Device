...
#define CONN_REQUEST_TIMEOUT 20000

#define HTTP_DATA_PREFIX "data="   //data key for post request
#define HTTP_DEVICEID_PREFIX "id=" //deviceid key for post request
...
public:
  ...
  int start(void);
  int check_connection(const char *url, int timeout = CONN_REQUEST_TIMEOUT);
  int send_device_data(const char *url, const char *number, int connection_type, DeviceData *device_data, int timeout = CONN_REQUEST_TIMEOUT);
  int upload_report(const char *url, unsigned char *base64_file, int timeout = CONN_REQUEST_TIMEOUT);
  int get_center_commands(const char *url, const char *number, int connection_type, CenterCommands *center_command, int timeout = CONN_REQUEST_TIMEOUT);
