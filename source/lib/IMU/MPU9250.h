...
#define MPU9250_SLAVE_ADDRESS 0x69 << 1 ///< MPU9250 Default I2C slave address
#define AK8963_SLAVE_ADDRESS 0x0C << 1 ///< AK8963 I2C slave address
...
class MPU9250 {
  public:
    MPU9250(I2C *i2c, uint8_t addr = MPU9250_SLAVE_ADDRESS);
    bool begin(void);
    bool searchDevice(void);
    void configMPU9250(uint8_t gfs = MPU9250_GFS_2000, uint8_t afs = MPU9250_AFS_2G);
    void configAK8963(uint8_t mode = AK8963_MODE_C100HZ, uint8_t mfs = AK8963_BIT_16);
    // bool checkDataReady();
    ...
    // struct Three_float readAccel();
    // struct Three_float readGyro();
    // struct Three_float readMagnet();
    struct imu_ypr_data readYPR();
  ...
  private:
    ...
    char readByte(uint8_t address, uint8_t register_addr);
    void writeI2c(uint8_t address, uint8_t register_addr, uint8_t data);
